...
<html lang="en">
  <head>
    ...
    <script type='` + MimeJs + `'>
      ...
      function onLoad() {
        // Existing mdrip session controller init
        ...
        // Initialize interactive code cells
        if (typeof initializeInteractiveCells === 'function') {
          initializeInteractiveCells();
        } else {
          console.error('initializeInteractiveCells function not found. Ensure app.js is loaded.');
        }
      }
    </script>
    <script type='application/javascript'>
      // This will be populated by the Go template with a JSON string
      // for window.appConfig.RunBlockURL etc.
      // Ensure AppConfigJSON is properly escaped to be a valid JS string literal.
      window.appConfig = JSON.parse('{{.AppConfigJSON | js}}');
    </script>
    <script type='` + MimeJs + `' src='/static/js/app.js' defer></script>
    <style>
      /* Basic styling for buttons, can be expanded or moved to cssInjected or a static CSS file */
